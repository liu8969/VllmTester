...
def print_statistics(results: List[Dict], total_time: float, concurrency: int):
    """增强统计信息显示，包含超时分析"""
    console = Console()
    ...
    per_request_tokens_per_sec = tokens_per_sec / concurrency if concurrency > 0 else 0
    ...
    stats_table.add_row("请求吞吐量", f"{requests_per_sec:.2f} 请求/秒")
    
    # 添加token相关指标
    if total_tokens > 0:
        stats_table.add_row("Token吞吐量", f"{tokens_per_sec:.2f} token/秒")
        stats_table.add_row("单个请求平均token吞吐量", 
                          f"{per_request_tokens_per_sec:.2f} token/秒/连接")
        stats_table.add_row("平均生成token数", 
                          f"{total_tokens/success_count:.1f} token/请求" if success_count > 0 else "N/A")
...
def main():
    ...
    # 打印统计信息，传入并发数
    print_statistics(results, total_time, args.max_concurrent)
